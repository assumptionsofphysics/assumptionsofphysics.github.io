...
Understanding what premises exactly lead to what conclusions is a crucial activity in the foundations of mathematics. The goal of Reverse Physics is to bring these types of discussions in the foundations of physics, strengthening the understanding of why particular mathematical structures are used in physics and what they represent. Let's see how this works.

As Harvey Friedman, the founder of Reverse Mathematics, stated: "when the theorem is derived from the right axioms, the axioms can be derived from the theorem." If the axioms and the theorem are logically equivalent, once a base theory is assumed, one can either take the axioms to be true or the theorem to be true. In set theory, for example, the Axiom of Choice and Zorn's Lemma are equivalent of the Zermelo-Frankel axioms. The basic insight is that this works for physics as well: **when the math is derived from the right physical assumptions, the physical assumptions can be derived from the math.**
